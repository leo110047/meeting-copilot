...
- macOS microphone live transcription uses `native/macos/MeetingCopilotSpeech.swift`.
- The helper uses `AVAudioEngine` for microphone capture and `Speech` for native streaming STT.
- macOS system-audio live transcription uses `ScreenCaptureKit` audio sample buffers and the same `Speech` request pipeline.
