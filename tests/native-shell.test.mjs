...
test("Tauri native shell is tray/status-item first", async () => {
  ...
  assert.match(desktopShellSource, /may_defer_post_meeting/);
  assert.match(desktopShellSource, /Startup cleanup approximates this from mixed\+stop_file/);
  assert.match(commandsAudio, /transcriber\.may_defer_post_meeting/);
  assert.match(commandsAudio, /Duration::from_secs\(31 \* 60\)/);
  ...
  assert.match(macBridge, /shouldDeferPostMeetingTranscription/);
  assert.match(macBridge, /drainDeferredPostMeetingJobs/);
  assert.match(macBridge, /post_meeting_/);
  assert.match(macBridge, /gracefulStopTimeoutSeconds/);
  assert.match(macBridge, /maxDeferredPostMeetingChunks = 1_200/);
  assert.match(macBridge, /postMeetingSecondsPerChunk/);
  assert.match(macBridge, /postMeetingChunkIndex/);
  assert.match(macBridge, /postMeetingDropReported/);
  assert.match(macBridge, /cleanupStalePostMeetingWhisperFiles/);
  assert.match(macBridge, /Six hours is intentionally wider than normal meetings/);
  assert.match(macBridge, /transcriptionQueue\.async \{ \[self\] in/);
  assert.doesNotMatch(macBridge, /\["mic", "mixed"\]\.contains\(source\), !requestMicrophoneAccessIfNeeded/);
  ...
  assert.match(windowsHelper, /DeferChunk/);
  assert.match(windowsHelper, /TranscribeDeferred/);
  assert.match(windowsHelper, /post_meeting_/);
  assert.match(windowsHelper, /MaxDeferredPostMeetingChunks = 1200/);
  assert.match(windowsHelper, /PostMeetingMillisecondsPerChunk = 8000/);
  assert.match(windowsHelper, /SweepStalePostMeetingFiles/);
  assert.match(windowsHelper, /postMeetingChunkIndex/);
  assert.match(windowsHelper, /CanDeferChunk/);
  assert.match(windowsHelper, /ReportDeferredChunkDropped/);
  assert.match(windowsHelper, /deferredDropReported/);
  assert.match(windowsHelper, /Six hours is intentionally wider than normal meetings/);
  ...
  assert.match(windowsHelper, /WaitForExit\(timeoutMs\)/);
  assert.match(windowsHelper, /postMeetingJobCount/);
  ...
  assert.match(runnerBlock, /public void Close\(int postMeetingJobCount\)/);
